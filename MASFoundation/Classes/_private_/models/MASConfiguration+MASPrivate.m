...
@implementation MASConfiguration (MASPrivate)
...
- (id)initWithConfigurationInfo:(NSDictionary *)info
{
    if(self = [super init])
    {
        ...
        self.isLoaded = (_configurationInfo_ && ([_configurationInfo_ count] > 0));
    }
    
    //DLog(@"\n\ncalled with configuration info: %@\n\n", _configurationInfo_);
...
+ (MASConfiguration *)instanceFromStorage
{
    ...
    //DLog(@"\n\ncalled and returning configuration: %@", configuration);
...
- (void)saveToStorage
{
    //DLog(@"called super and class is: %@", [self.class description]);
    ...
    //DLog(@"called and instance is: %@", [self debugDescription]);
}
...
{
    //DLog(@"called with current info: %@", [self debugDescription]);
    ...
    // Configuration
    if(_configurationInfo_) [keyChainService setConfiguration:_configurationInfo_];
}
...
- (BOOL)isLoaded
{
    NSNumber *isLoadedNumber = objc_getAssociatedObject(self, &MASConfigurationIsLoadedPropertyKey);
    
    return [isLoadedNumber boolValue];
}


- (void)setIsLoaded:(BOOL)isLoaded
{
    objc_setAssociatedObject(self, &MASConfigurationIsLoadedPropertyKey, [NSNumber numberWithBool:isLoaded], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (NSArray *)gatewayCertificates
{
    ...
    //DLog(@"Gateway certificates: %@", (NSArray *)gatewayInfo[MASGatewayCertificatesKey]);
...
- (NSArray *)gatewayCertificatesAsDERData
{
    ...
    //DLog(@"Gateway certificates as DER data: %@", certificates);
...
- (NSArray *)gatewayCertificatesAsPEMData
{
    ...
    //DLog(@"Gateway certificates as PEM data: %@", certificates);
...
- (NSString *)endpointPathForKey:(NSString *)endpointKey
{
    ...
    //DLog(@"\n\nFor key: %@ found endpoint path: %@\n\n", endpointKey, endpointPath);
